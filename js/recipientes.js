function calcularepFcn() {
    ...
    Vtapas = Math.PI/12*(Math.pow(diametro + 2*espesortapareal,3) - Math.pow(diametro,3));
        ...
        vala = diametro/2;
        theta = Math.acos((vala-nivel)/vala);
        Vliquido = Math.pow(vala,2)*longitud*(theta-Math.sin(theta)*Math.cos(theta));
        document.querySelector('#volumliquido').value = Vliquido.toFixed(0);
        ...
        equipolleno=pesoliquido+pesoequipo;
        document.querySelector('#equipolleno').value = equipolleno.toFixed(0);
